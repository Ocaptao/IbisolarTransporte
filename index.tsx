...
let userProfilesCollection: CollectionReference<UserProfile>;
let tripsCollection: CollectionReference<Trip>;
...
try {
    ...
    auth = getAuth(app);
    db = getFirestore(app);
    userProfilesCollection = collection(db, "userProfiles") as CollectionReference<UserProfile>;
    tripsCollection = collection(db, "trips") as CollectionReference<Trip>;
...
interface Trip {
    ...
    // updatedAt?: Timestamp; // Consider adding for tracking updates
}
        ...
        try {
            ...
            if (userProfileDoc.exists()) {
                ...
                if (auth.currentUser && auth.currentUser.uid === user.uid) {
                    ...
                    console.warn("User session changed (e.g., logged out or different user signed in) while fetching profile for UID:", user.uid, ". Aborting UI update for this stale session. The new auth state will be handled.");
...
async function handleTripFormSubmit(event: Event) {
    ...
    const fuelEntriesFromForm: FuelEntry[] = [];
    ...
    const kmInitialVal = parseFloat(formData.get('kmInitial') as string) || 0;
    const kmFinalVal = parseFloat(formData.get('kmFinal') as string) || 0;
    const kmDrivenVal = (kmFinalVal > kmInitialVal) ? kmFinalVal - kmInitialVal : 0;

    const arla32CostVal = parseFloat(formData.get('arla32Cost') as string) || 0;
    const tollCostVal = parseFloat(formData.get('tollCost') as string) || 0;
    const commissionCostVal = parseFloat(formData.get('commissionCost') as string) || 0;
    const otherExpensesVal = parseFloat(formData.get('otherExpenses') as string) || 0;

    const totalExpensesCalculated = totalFuelCostCalculated + arla32CostVal + tollCostVal + otherExpensesVal + commissionCostVal;
    const freightValueVal = parseFloat(formData.get('freightValue') as string) || 0;
    const netProfitVal = freightValueVal - totalExpensesCalculated;

    const tripDataObjectFromForm = {
        userId: loggedInUser.uid,
        driverName: (formData.get('driverName') as string).trim() || loggedInUserProfile.username,
...
function confirmDeleteTrip(tripId: string, driverNameForConfirm: string) {
    ...
    const tripToDelete = trips.find(t => t.id === tripId) || 
                        (adminView.style.display === 'block' ? trips.find(t=>t.id === tripId) : null); 
...
async function updateAdminSummary(filterStartDate?: string, filterEndDate?: string) {
    ...
    try {
        ...
        querySnapshot.forEach((doc) => {
            const trip = doc.data() as Trip; // Data is already Trip due to typed collection
            totalTrips++;
            totalFreight += trip.freightValue;
...
async function handleEditUserFormSubmit(event: Event) {
    ...
    try {
        ...
        await updateDoc(userProfileRef, { role: newRole } as Record<string, any>); // Cast for updateDoc
