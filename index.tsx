...
async function handleRegister(event: Event) {
    ...
    try {
        ...
        if (error.code === 'auth/email-already-in-use') {
            ...
            showFeedback(registerFeedback, "Erro ao registrar. Tente novamente.", "error");
        }
    }
}
...
async function handleLogin(event: Event) {
    ...
    try {
        ...
        if (error.code === 'auth/user-not-found' || error.code === 'auth/wrong-password' || error.code === 'auth/invalid-credential') {
            ...
            showFeedback(loginFeedback, "Erro ao tentar fazer login. Tente novamente.", "error");
        }
    }
}
    ...
    if (user) {
        ...
        try {
            ...
            if (userProfileDoc.exists()) {
                ...
                await handleLogout();
                showFeedback(loginFeedback, "Erro ao carregar perfil do usuário. Tente novamente.", "error");
            }
        } catch (error) {
            console.error("Error fetching user profile:", error);
            await handleLogout(); // Forçar logout em caso de erro
            showFeedback(loginFeedback, "Erro ao carregar dados do usuário. Tente novamente.", "error");
        }
    } else {
        loggedInUser = null;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!app || !auth || !db) {
        console.error("Firebase não foi inicializado corretamente. O aplicativo pode não funcionar.");
